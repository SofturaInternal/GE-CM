...
for i in range(0,row):
    ...
    if Product_Descriptor_Level3 == repo:
       ...
       query = "INSERT INTO cmdb_design.repo (ReleaseID, Repo_ID, Repo_URL, Status, DTR, Latest_Commit_No, VersionFixed) VALUES (%s, %s, %s, %s, %s, %s, %s)"
       print(query) 
       values = (j,k,Repo_URL, Status , DTR, Repo_Latest_Commit_No,VersionFixed)
       print(values)
       try:
       ...
       for item in r:
			...
			continue
                print "Inserting into Repo Commits table"
      	        query = "INSERT INTO cmdb_design.repocommits (Repo_ID, Commit_No,DEF_DSRVW,DSRVW_Status) VALUES (%s, %s,%s,%s)"
                print(query) 
                values = (Repo_URL, commit_number,edr, DSRVW_Status)
                print(values)
                try:
                   cursor.execute(query, values)
